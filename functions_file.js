...
function compositeMatchScore(pattern, userInput, tolerance = 0.72) {
    ...
    // total word match score, use unique set to avoid duplicate words scoring four fold.
    ...
    for (const patternToken of uniquePatternTokens) {
        for (const userToken of uniqueUserTokens) {
            const similarity = stringSimilarity.compareTwoStrings(userToken, patternToken);
            if (similarity >= 0.3) wordMatchScore += similarity;
        }
    }
